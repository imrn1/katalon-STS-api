...
WS.sendRequestAndVerify(findTestObject('Postman/Transaction/CashBackCredit', [
		('ext_transaction_id') : transaction_id
        , ('sender_account_number') : "$sender_account_number", ('sender_wallet_number') : "$sender_wallet_number", ('receiver_wallet_number') : "$receiver_wallet_number"
        , ('amount') : "$amount", ('currency_code') : "$currency_code", ('description') : "$description", ('source_type') : "$source_type"
        , ('channel_type') : "$channel_type", ('hash_key') : "$hash_key"]))


CustomKeywords.'writeFile.WriteFile.writeExel'('CashBackCredit', "$process_description", "$expected_status", "$expected_code", 
    "$expected_message", 'CashBackCredit', "$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'CashBackCredit'])
