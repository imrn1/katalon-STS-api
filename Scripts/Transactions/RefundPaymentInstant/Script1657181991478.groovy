...
if (isNullOrEmpty(correlation_id)) {
    ...
    RefundPaymentResponse = WS.sendRequestAndVerify(findTestObject('Postman/Transaction/RefundPayment Instant', [('ext_transaction_id') : transaction_id
                , ('tx_correlation_id') : correlation_id, ('currency_code') : "$currency_code", ('refund_amount') : "$refund_amount"
                , ('description') : "$description"]))

    CustomKeywords.'writeFile.WriteFile.writeExel'('RefundPaymentInstant', "$process_description", "$expected_status", "$expected_code", 
        "$expected_message", fileName = 'RefundPaymentInstant',"$expected_ResponseStatusCode")
    ...
    WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'RefundPaymentInstant'])
} else {
    if (correlation_id == 'loop') {
        ...
        if ("$refund_amount" == '100') {
            ...
            RefundPaymentResponse = WS.sendRequestAndVerify(findTestObject('Postman/Transaction/RefundPayment Instant', [
				('ext_transaction_id') : transaction_id
                        , ('tx_correlation_id') : correlation_id , ('currency_code') : "$currency_code",
						 ('refund_amount') : "$refund_amount"
                        , ('description') : "$description"]))

            CustomKeywords.'writeFile.WriteFile.writeExel'('RefundPaymentInstant', "$process_description", "$expected_status", 
                "$expected_code", "$expected_message", fileName = 'RefundPaymentInstant',"$expected_ResponseStatusCode")
            ...
            for (int i = 0; i < GlobalVariable.tx_correlation_id.size(); i++) {
                ...
                transaction_id = GlobalVariable.uniqe_id
				correlation_id = GlobalVariable.tx_correlation_id[i]
				...
				if(Float.parseFloat(refund_amount) == 0) { // amount : 0 olan ödeme iadesinden beklenen sonuçlar bu alanda setlendi
					...
					def expected_code = "315" 
					def expected_message = "Geçersiz tutar girişi."
