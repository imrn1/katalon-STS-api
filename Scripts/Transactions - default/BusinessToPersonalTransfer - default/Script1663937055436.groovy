...
WebUI.callTestCase(findTestCase('_Custom Cases/Wallet Info'),
	[
		...
		('beforeTransaction') : 1,
		('serviceName') : 'BusinessToPersonalTransfer'
	])
...
def BusinessToPersonalTransferResponse = WS.sendRequestAndVerify(findTestObject('Postman/Transaction/BusinessToPersonalTransfer',[
...
CustomKeywords.'writeFile.WriteFile.writeExel'('BusinessToPersonalTransfer',"$process_description","$expected_status","$expected_code",
	"$expected_message", fileName="BusinessToPersonalTransfer","$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'BusinessToPersonalTransfer'] )
