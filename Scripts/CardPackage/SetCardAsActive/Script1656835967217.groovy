...
def SetCardAsActiveresponse =  WS.sendRequestAndVerify(findTestObject('Postman/CardPackage/SetCardAsActive', [
		"account_number": "${account_number}",
		"wallet_number": "${wallet_number}",
		"card_no": "${card_no}"
	]))
 
if(WS.getElementPropertyValue(SetCardAsActiveresponse, "status") == 0) {
	// kart aktif edilirse işlemlerde kullanılmak üzere listeye eklenir
	if(GlobalVariable.activeCardNo == null) {
		GlobalVariable.activeCardNo = []
	}
	GlobalVariable.activeCardNo.add("${card_no}")
...
CustomKeywords.'writeFile.WriteFile.writeExel'('SetCardAsActive',"$process_description","$expected_status","$expected_code",
	"$expected_message","SetCardAsActive","$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'SetCardAsActive'] )
