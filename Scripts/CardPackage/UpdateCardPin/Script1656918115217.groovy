...
def UpdateCardPinResponse = WS.sendRequestAndVerify(findTestObject('Postman/CardPackage/UpdateCardPin', [
		"card_no": "${card_no}",
		"old_pin": "${old_pin}",
		"new_pin": "${new_pin}"
	]))

CustomKeywords.'writeFile.WriteFile.writeExel'('UpdateCardPin',"$process_description","$expected_status",
	"$expected_code","$expected_message","UpdateCardPin","$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'UpdateCardPin'] )
