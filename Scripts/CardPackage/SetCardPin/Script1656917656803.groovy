...
def SetCardPinResponse = WS.sendRequestAndVerify(findTestObject('Postman/CardPackage/SetCardPin',[
		"card_no": "${card_no}",
		"new_pin": "${new_pin}"
	]))

CustomKeywords.'writeFile.WriteFile.writeExel'('SetCardPin',"$process_description","$expected_status","$expected_code",
	"$expected_message","SetCardPin","$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'SetCardPin'] )
