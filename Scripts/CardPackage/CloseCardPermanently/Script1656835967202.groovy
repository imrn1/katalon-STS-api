...
def CloseCardPermanentlyResponse = WS.sendRequestAndVerify(findTestObject('Postman/CardPackage/CloseCardPermanently'[
		"account_number": "${account_number}",
		"wallet_number": "${wallet_number}",
		"card_no": "${card_no}"
	]))

if (WS.getElementPropertyValue(CloseCardPermanentlyResponse, 'status') == 0) {
	// kart aktif edilirse işlemlerde kullanılmak üzere listeye eklenir
	if (GlobalVariable.closedPermanentlyCardNo == null) {
		GlobalVariable.closedPermanentlyCardNo = []
	}
	
	GlobalVariable.closedPermanentlyCardNo.add("$card_no")
...
CustomKeywords.'writeFile.WriteFile.writeExel'('CloseCardPermanently',"$process_description","$expected_status","$expected_code",
	"$expected_message","CloseCardPermanently","$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'CloseCardPermanently'] )
