...
def SetCardAsLostResponse = WS.sendRequestAndVerify(findTestObject('Postman/CardPackage/SetCardAsLost', [
		"account_number": "${account_number}",
		"wallet_number": "${wallet_number}",
		"card_no": "${card_no}"
	]))

if (WS.getElementPropertyValue(SetCardAsLostResponse, 'status') == 0) {
	// kart aktif edilirse işlemlerde kullanılmak üzere listeye eklenir
	if (GlobalVariable.lostCardNo == null) {
		GlobalVariable.lostCardNo = []
	}
	
	GlobalVariable.lostCardNo.add("$card_no")
...
CustomKeywords.'writeFile.WriteFile.writeExel'('SetCardAsLost',"$process_description","$expected_status","$expected_code","$expected_message",
	'SetCardAsLost',"$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'SetCardAsLost'] )
