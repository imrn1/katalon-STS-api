...
def CloseCardAsStolenResponse = WS.sendRequestAndVerify(findTestObject('Postman/CardPackage/CloseCardAsStolen',[
...
if (WS.getElementPropertyValue(CloseCardAsStolenResponse, 'status') == 0) {
	// kart aktif edilirse işlemlerde kullanılmak üzere listeye eklenir
	if (GlobalVariable.stolenCardNo == null) {
		GlobalVariable.stolenCardNo = []
	}
	
	GlobalVariable.stolenCardNo.add("$card_no")
...
CustomKeywords.'writeFile.WriteFile.writeExel'('CloseCardAsStolen',"$process_description","$expected_status","$expected_code",
	"$expected_message",'CloseCardAsStolen',"$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'CloseCardAsStolen'] )
