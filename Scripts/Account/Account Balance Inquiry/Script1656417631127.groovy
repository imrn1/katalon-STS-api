...
WS.sendRequestAndVerify(findTestObject('Postman/Account/AccountBalanceInquiry', [('account_number') : "$account_number"]))
 
//  serviceName, processDescription,  expected_status, expected_code,  expected_message
CustomKeywords.'writeFile.WriteFile.writeExel'('AccountBalanceInquiry',"$process_description","$expected_status","$expected_code",
	"$expected_message",'AccountBalanceInquiry', "$expected_ResponseStatusCode")
...
WebUI.callTestCase(findTestCase('_Verify Cases/verify payloadNotBoolean'), [('servisName') : 'AccountBalanceInquiry'] )
